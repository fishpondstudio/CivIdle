export const ZH_CN = {
   ...
   BritishMuseumDesc: "建造完成后，可以变更为其他文明的奇观（蓬皮社文艺中心与自然奇观除外。仅有一次变更机会，请慎重考虑）",
   ...
   BuildingLevelBoostFromElectrification: "来自电气化中的额外建筑等级",
   ...
   CathedralOfBrasiliaDesc: "如果周围 2 地块范围的每个建筑形成一个完整的生产链（上游各分支也计入在内），即一个建筑生产的资源被另一个建筑消耗，那么参入该完整生产链中的每种建筑为所有同类型建筑提供 +X 生产乘数。X = 生产链的总长度（最大值为18）",
   CathedralOfBrasiliaWarningHTML: "为了形成生产链，一个建筑生产的资源除了一种资源外，其他所有资源都需要被另一个建筑消耗",
   Census: "劳动者统计",
   ...
   ElectrificationStatusDesc: "无论建筑是否需要电力，都存在电气化可能。不同之处在于，需要电力的建筑可以从电气化中获得额外的 +5 建筑等级",
   ...
   LiberalismLevel5DescHTMLV2: "所有可电气化的建筑获得额外<b> +5 建筑等级</b>",
   ...
   SwissBank: "瑞士银行",
   SwissBankDescV3: "从所有可与玩家交易的资源（旅馆和联动的仓库）中选择一种资源转换为💵瑞士货币。每个周期基础产出1个💵瑞士货币（可被生产乘数影响），该奇观可被升级，并且每一次额外升级增加 1 基础产出（无论基础产出提升还是生产乘数提升都只是提升每周期等价兑换最大总数量）。瑞士银行可以存储无限量的💵瑞士货币",
   Sword: "剑",
   SwordForge: "铸剑铺",
