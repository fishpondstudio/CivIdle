export const ZH_CN = {
   ...
   CathedralOfBrasiliaDesc:
      "如果周围 2 地块范围的每个建筑形成一个完整的生产链，即范围内一个建筑生产的资源被范围内另一个建筑消耗（范围当中相同产出不同建筑也会被归入完整的生产链中），那么参入该完整的生产链中的每种建筑为其所有同类型建筑提供 +X 生产乘数。X = 生产链的总长度（建筑种类数量，最大值为18。奇观、旅馆、仓库、市集、克隆工厂和克隆实验室除外）",
   ...
   GenerateOneTimeWarpThatRefillsAllAvailableWarpStorage: "一次性填满扭曲时间存储",
   Genetics: "遗传学",
   GenghisKhan: "成吉思汗",
   ...
   GrandBazaar: "大巴扎",
   GrandBazaarDesc: "该奇观可管理你的所有市集，与该奇观相邻的市集拥有不同的交易项。与该奇观相邻的商队旅馆，获得 +5 生产与存储乘数",
   ...
   OpetV2: "欧佩特：狮身人面像额外为周围 2 地块范围，所有 II 阶及以上建筑，提供 +N 生产等级（ N 为其相邻相同类型建筑数量）",
   OpticalFiber: "光纤",
   ...
   Petra: "佩特拉",
   PetraDescV3: "解锁更多扭曲倍速。该奇观可以升级，每一级提供 1 小时的扭曲存储。当你离线时生成扭曲时间，你可以用来加速你的帝国或携带最多 4 小时（14.4K）扭曲时间到重生后的地图（可在右上角的选项-游戏设置-离线生产栏里分配离线设置）",
   ...
   PlusXGreatPeopleFromTheCurrentAge: "诞生 %{count} 当前时代伟人",
