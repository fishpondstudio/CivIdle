export const ZH_CN = {
   ...
   CentrePompidouDesc:
      "建造完成后，所有建筑物都将获得 +1 的生产和 +2 存储乘数。如果当前的游玩达到信息时代，下一次游玩是不同的文明，那么这个奇观将持续存在。对于每次以独一无二的文明而达到信息时代的重生，奇观在重生时获得 +1 等级，每个级别提供 +1 生产和 +2 存储乘数。这个奇观的价值不计入帝国的总价值中，大英博物馆无法转变为这个奇观。",
   ...
   EastIndiaCompanyDescV2:
      "这个奇观累积您与玩家完成交易的总贸易价值。每消耗 2000 贸易价值获得一个周期, 使所有建筑每与一个运转的商队旅馆相邻获得 +0.5 生产乘数。该奇观可被升级，并且每一次额外升级获得 +0.5 生产乘数。当您完成了另一个玩家的一笔交易请求或您自己的一笔交易请求被完成时，通过交易所获得物品其价值才被计入（税收也被计入）。多次交易延长持续时间，可多重叠加加成。",
   Education: "教育学",
