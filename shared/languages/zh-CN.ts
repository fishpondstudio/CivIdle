export const ZH_CN = {
   ...
   CathedralOfBrasiliaDesc: "如果周围 2 地块范围的每个建筑形成一个完整的生产链，即范围内一个建筑生产的资源被范围内另一个建筑消耗（范围当中相同产出不同建筑也会被归入完整的生产链中），那么参入该完整的生产链中的每种建筑为其所有同类型建筑提供 +X 生产乘数。X = 生产链的总长度（建筑种类数量，最大值为18。奇观、旅馆、仓库、市场、克隆工厂和克隆实验室除外）",
   ...
   WTOVoteEndIn: "结束前可以随时更改投票。投票结束剩余时间：<b>%{time}</b>",
