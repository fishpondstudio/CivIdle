export const ZH_TW = {
   ...
   EuphratesRiverDesc: 
      "每 10% 從事生產（非運輸）的忙碌工人為所有不生產工人的建築物提供 +1 生產乘數（最大值 = 解鎖時代數 / 2）。當旁邊建造空中花園時，空中花園解鎖後每個時代 獲得+1效果。發現後，在所有相鄰的沒有沉積物的方塊上產生水",
   ...
   ProductionPriorityDescV4:
      "生產優先等級決定建築的運輸和生產順序——較高數字表示建築會先於其他建築進行運輸和生產。然而，較高的優先等級並不保證建築會優先運輸資源。例如：煤電廠 > 煤礦 > 鋼廠的優先順序中，鋼廠將在同一周期內獲得所有煤礦生產的煤",
   ...
   TransportPlanCacheDescHTML:
      "每個週期中，每個建築物都會根據其設置計算最佳運輸計劃——此過程需要高 CPU 功率。啟用此功能將嘗試快取運輸計劃的結果（如果其仍然有效），從而降低 CPU 使用率和幀率下降。<b>實驗性功能</b>",
