export const RU = {
   CurrentLanguage: "Русский",
   TranslatorCredit: "WingedLeo",
   Translators: "Переводчики",
   Language: "Язык",
   TechnologyPrerequisite: "Требования",
   TechnologyNoPrerequisite: "Нет Требований",
   Fire: "Огонь",
   Logging: "Лесозаготовка",
   Shelter: "Укрытие",
   StoneTools: "Каменные Инструменты",
   Masonry: "Кирпичная Кладка",
   Herding: "Стадо",
   Farming: "Фермерство",
   BronzeTech: "Бронза",
   Housing: "Жильё",
   Counting: "Счёт",
   HorsebackRiding: "Верховая Езда",
   Metallurgy: "Металлургия",
   Stateship: "Государство",
   Arithmetic: "Арифметика",
   Writing: "Письменность",
   Shamanism: "Шаманство ",
   RoadAndWheel: "Дорога и Колесо",
   Astronomy: "Астрономия",
   IronTech: "Железо",
   Construction: "Строительство",
   Music: "Музыка",
   Cloth: "Ткань",
   GarmentWorkshop: "Швейная Мастерская",
   FurnitureWorkshop: "Мебельная Мастерская",
   Garment: "Одежда",
   Calendar: "Календарь",
   Engineering: "Инженерия",
   Gold: "Золото",
   PreciousMetal: "Драгоценный Металл",
   Market: "Рынок",
   MarketDesc: "Обменивайте один ресурс на другой, доступные ресурсы обновляются каждый час",
   CityState: "Город-Государство",
   Literature: "Литература",
   Machinery: "Оборудование",
   LandTrade: "Сухопутная Торговля",
   Politics: "Политика",
   Oil: "Нефть",
   NaturalGas: "Природный Газ",
   NaturalGasWell: "Скважина Природного Газа",
   OilRefinery: "Нефтеперерабатывающий Завод",
   Petrol: "Бензин",
   Aluminum: "Алюминий",
   Theater: "Театр",
   KnightCamp: "Лагерь рыцарей",
   CannonWorkshop: "Пушечная мастерская",
   DynamiteWorkshop: "Мастерская Динамита",
   LensWorkshop: "Мастерская Линз",
   Lens: "Линзы",
   GunpowderMill: "Пороховая Фабрика",
   Cannon: "Пушка",
   Dynamite: "Динамит",
   Gunpowder: "Порох",
   Coal: "Уголь",
   Silicon: "Кремний",
   Sand: "Песок",
   Knight: "Рыцарь",
   Architecture: "Архитектура",
   Democracy: "Демократия",
   Autocracy: "Автократия",
   Religion: "Религия",
   Geography: "Георграфия",
   Physics: "Физика",
   Feudalism: "Феодализм",
   Theocracy: "Теократия",
   Education: "Образование",
   Navigation: "Навигация",
   CivilService: "Госслужба",
   Cathedral: "Кафедральный Собор",
   Printing: "Печать",
   Exploration: "Разведка",
   Banking: "Банковское Дело",
   University: "Университет",
   Museum: "Музей",
   Culture: "Культура",
   Courthouse: "Здание Суда",
   Stadium: "Стадион",
   Parliament: "Парламент",
   Sports: "Спорт",
   PublishingHouse: "Издательство",
   MagazinePublisher: "Издатель Журналов",
   HolyEmpire: "Священная Империя",
   Renaissance: "Ренессанс",
   PrintingPress: "Печатный Станок",
   PrintingHouse: "Типография",
   Book: "Книга",
   Chemistry: "Химия",
   Enlightenment: "Просвещение",
   Firearm: "Огнестрельное Оружие",
   Optics: "Оптика",
   PrivateOwnership: "Частная Собственность",
   Revolution: "Революция",
   Constitution: "Конституция",
   Library: "Библиотека",
   Engine: "Двигатель",
   SteamEngine: "Паровой Двигатель",
   LocomotiveFactory: "Локомотивный Завод",
   TankFactory: "Танковый Завод",
   Tank: "Танк",
   Train: "Поезд",
   Capitalism: "Капитализм",
   Alloy: "Сплав",
   Rifling: "Нарезка Ствола",
   Colonialism: "Колониализм",
   RapidFire: "Шквальный Огонь",
   Newspaper: "Газета",
   Magazine: "Журнал",
   Journalism: "Журналистика",
   Drilling: "Бурение",
   Railway: "Железная Дорога",
   Olympics: "Олимпиада",
   Refinery: "Нефтеперерабатывающий Завод",
   Combustion: "Воспламенение ",
   Imperialism: "Империализм",
   StockMarket: "Фондовый Рынок",
   Electricity: "Электричество",
   Electrolysis: "Электролиз",
   GasPipeline: "Газопровод",
   Urbanization: "Урбанизация",
   Aeroplane: "Аэроплан",
   AtomTheory: "Теория атомов",
   Synthetics: "Синтетика",
   MotionPicture: "Кинокартина",
   Assembly: "Установка",
   Medicine: "Медицина",
   Hospital: "Больница",
   Greenhouse: "Теплица",
   Rocketry: "Рокетная Промышленность",
   Semiconductor: "Полупроводник",
   Vaccine: "Вакцина",
   Refrigeration: "Холодильник",
   SpaceProgram: "Космическая Программа",
   Television: "Телевидение",
   Tourism: "Туризм",
   Skyscrapper: "Небоскрёб",
   Computer: "Компьютер",
   Globalization: "Глобализация",
   UnitedNation: "Объединенная Нация",
   Metropolitan: "Метрополитен",
   NuclearFission: "Ядерное Деление",
   Internet: "Интернет",
   Nanotechnology: "Нанотехнологии",
   Genetics: "Генетика",
   Conglomerate: "Конгломерат",
   Robotics: "Робототехника",
   SocialNetwork: "Социальная Сеть",
   Smartphone: "Смартфон",
   UniversalHealthcare: "Всеобщее Здравоохранение",
   VentureCapital: "Инвестиции",
   ArtificialIntelligence: "Искуственный Интелект",
   CloudComputing: "Облачные Вычесления",
   VirtualReality: "Виртуальная Реальность",
   Cryptocurrency: "Криптовалюта",
   ViewTechnology: "Посмотреть",
   Headquarter: "Центральный Офис",
   UnexploredTile: "Неисследованная Плитка",
   Tile: "Плитка",
   Deposit: "Месторождение",
   UniqueBuildings: "Уникальные здания",
   RequiredDeposit: "Требуется Месторождение",
   NoDepositDesc:
      "На этой плитке нет месторождения. Другие месторождения могут быть обнаружены, когда вы откроете новые технологии",
   UnlockBuilding: "Открыть",
   UpgradeBuilding: "Улучшить",
   RevealDeposit: "Открывает",
   BuildingMultipliers: "Усиление",
   BuildingANew: "Построить",
   BuildingSearchText: "Введите название здания для поиска",
   Worker: "Рабочий",
   WheatFarm: "Пшеничная ферма",
   FlourMill: "Мельница",
   OlivePlantation: "Оливковая плантация",
   Vineyard: "Виноградник",
   Caravansary: "Базар",
   CaravansaryDesc: "Обменивайтесь ресурсами с другими игроками и предоставляйте дополнительное хранилище",
   Winery: "Винодельня",
   OilPress: "Пресс для Масла",
   Wine: "Вино",
   Paper: "Бумага",
   Olive: "Олива",
   LivestockFarm: "Ферма для Скота",
   Stable: "Конюшня",
   ChariotWorkshop: "Колесничная Мастерская",
   PaperMaker: "Завод Бумаги",
   Pizzeria: "Пиццерия",
   Bakery: "Пекарня",
   Bread: "Хлеб",
   CheeseMaker: "Сыроварня",
   WritersGuild: "Гильдия Писателей",
   PaintersGuild: "Гильдия Художников",
   Painting: "Картина",
   MusiciansGuild: "Гильдия Музыкантов",
   ActorsGuild: "Гильдия Актёров",
   Cheese: "Сыр",
   Hut: "Хижина",
   House: "Дом",
   ViewMenu: "Вид",
   OptionsMenu: "Опции",
   HelpMenu: "Помощь",
   Census: "Перепись Населения",
   OptionsUseModernUI: "Защита Глаз",
   About: "О CivIdle",
   ResearchMenu: "Исследования",
   CityViewMap: "Город",
   ConsumeResource: "Потребляет: %{resource}",
   Consume: "Потребляет",
   ProduceResource: "Производит: %{resource}",
   Produce: "Производит",
   BaseMultiplier: "Базовый Множитель",
   BaseProduction: "Базовое Производство",
   BaseConsumption: "Базовое Потребление",
   ProductionMultiplier: "Множитель Производства",
   ConsumptionMultiplier: "Множитель Потребления",
   WorkerMultiplier: "Рабочая Мощность",
   SourceResearch: "Исследования: %{tech}",
   SourceGreatPerson: "Великий Человек: %{person}",
   SourceGreatPersonPermanent: "Постоянный Великий Человек: %{person}",
   LevelX: "Уровень %{level}",
   Level: "Уровень",
   Workers: "Рабочие",
   WorkersRequiredInput: "Транспортировка",
   ResourceFromBuilding: "%{resource} из %{building}",
   WorkersRequiredOutput: "Производство",
   WorkersRequiredForProductionMultiplier: "Производственная Мощность за Рабочего",
   WorkersRequiredForTransportationMultiplier: "Транспортировочная Мощность за Рабочего",
   WorkersRequiredBeforeMultiplier: "Требуемая Рабочая Мощность",
   WorkersRequiredAfterMultiplier: "Требуется Рабочих",
   AdjustBuildingCapacity: "Производственная Мощность",
   Storage: "Хранилище",
   TotalStorage: "Общее Хранилище",
   StorageBaseCapacity: "Базовая Мощность",
   StorageMultiplier: "Множитель Хранилища",
   StorageUsed: "Хранилища Использовано",
   StorageLeft: "Хранилища Свободно",
   Wheat: "Пшеница",
   Meat: "Мясо",
   Wood: "Дерево",
   Lumber: "Доска",
   Brick: "Кирпич",
   Horse: "Лошадь",
   Marble: "Мрамор",
   Stone: "Камень",
   Water: "Вода",
   Copper: "Медь",
   Iron: "Железо",
   Tool: "Инструменты",
   Alcohol: "Алкоголь",
   Blacksmith: "Кузница",
   IronForge: "Кузница Железа",
   SwordForge: "Кузница Мечей",
   Brickworks: "Кирпичный Завод",
   Marbleworks: "Мраморный Завод",
   SteelMill: "Сталелитейный Завод",
   Steamworks: "Паровая Мастерская",
   Steel: "Сталь",
   StoneTool: "Каменные Инструменты",
   StoneQuarry: "Каменный Карьер",
   LoggingCamp: "Лесозаготовительный Лагерь",
   LumberMill: "Лесопильный Завод",
   Glassworks: "Стеклянной Завод",
   Glass: "Стекло",
   IronMiningCamp: "Железодобывающий Лагерь",
   CopperMiningCamp: "Меднодобывающий Лагерь",
   CoalMine: "Угольная Шахта",
   Sandpit: "Песчаный Карьер",
   GoldMiningCamp: "Золотодобывающий Лагерь",
   OilWell: "Нефтяная Скважина",
   AluminumSmelter: "Алюминиевый Завод",
   CoalPowerPlant: "Угольная Электростанция",
   CoinMint: "Монетный Двор",
   Bank: "Банк",
   BondMarket: "Рынок Облигаций",
   StockExchange: "Фондовая биржа",
   Stock: "Акции",
   Coin: "Монета",
   Banknote: "Банкнота",
   Bond: "ОБлигация",
   Science: "Наука",
   Faith: "Вера",
   Philosophy: "Философия",
   Law: "Закон",
   Shrine: "Храм",
   Church: "Церковь",
   Mosque: "Мечеть",
   School: "Школа",
   WorkersAvailable: "Рабочих Доступно",
   WorkerHappinessPercentage: "Множитель Счастья",
   WorkerPercentagePerHappiness: "%{value}% Множителя за Каждое Счастье",
   WorkersAvailableBeforeHappinessMultiplier: "Работники, До Множителя Счастья",
   WorkersAvailableAfterHappinessMultiplier: "Работники, После Множителя Счастья",
   WorkersBusy: "Рабочих Занято",
   WorkerScienceProduction: "Производство Науки Рабочими",
   ScienceFromIdleWorkers: "Наука от Незанятых Рабочих",
   BuilderCapacity: "Мощность Строительства",
   SciencePerIdleWorker: "За Незанятого Рабочего",
   ScienceFromBusyWorkers: "Наука от Занятых Рабочих",
   SciencePerBusyWorker: "За Занятого Рабочего",
   TechGlobalMultiplier: "Усиление",
   UnlockableResearch: "Доступные Исследования",
   Research: "Исследование",
   TechHasBeenUnlocked: "%{tech} было открыто",
   StoneAge: "Каменный Век",
   BronzeAge: "Бронзовый Век",
   IronAge: "Железный Век",
   ClassicalAge: "Классический Век",
   MiddleAge: "Средний Век",
   RenaissanceAge: "Ренессанс",
   IndustrialAge: "Промышленность",
   WorldWarAge: "Мировые Войны",
   ColdWarAge: "Холодная Война",
   InformationAge: "Информационный Век",
   Name: "Название",
   View: "Посмотреть",
   UnlockTechProgress: "Прогресс",
   BoostDescription: "+%{value} %{multipliers} для %{buildings}",
   Hammurabi: "Хаммурапи",
   TangOfShang: "Тан из Шан",
   TangOfShangDesc: "+%{value} Науки от Незанятых Рабочих",
   RamessesII: "Рамзес II",
   RamessesIIDesc: "+%{value} Множитель Мощности Стоительства",
   Agamemnon: "Агамемнон",
   SargonOfAkkad: "Саргон Аккадский",
   Dido: "Дидона",
   DukeOfZhou: "Герцог Чжоу",
   Ashurbanipal: "Ашшурбанипал",
   NebuchadnezzarII: "Навуходоносор II",
   LaoZi: "Лао Цзы",
   Solon: "Солон",
   CyrusII: "Кир II",
   DariusI: "Дарий I",
   Confucius: "Конфуций",
   Socrates: "Сократ",
   SocratesDesc: "+%{value} Науки от Занятых Рабочих",
   Aeschylus: "Эсхил",
   Protagoras: "Протагор",
   Herodotus: "Геродот",
   Hippocrates: "Гиппократ",
   Plato: "Платон",
   Aristotle: "Аристотель",
   AlexanderIII: "Александр III",
   Ashoka: "Ашока",
   Hannibal: "Ганнибал",
   QinShiHuang: "Цинь Шихуанди",
   Justinian: "Юстиниан",
   Charlemagne: "Шарльман",
   HarunAlRashid: "Харун аль-Рашид",
   WuZetian: "Императрица Ву Цзетянь",
   WuZetianDesc: "+%{value} Множитель Мощности Транспортировки",
   Rurik: "Рюрик",
   RurikDesc: "+%{value} Счастья",
   LeonardoDaVinci: "Леонардо да Винчи",
   MartinLuther: "Мартин Лютер",
   WilliamShakespeare: "Уильям Шекспир",
   ReneDescartes: "Рене Декарт",
   ZhengHe: "Чжэн Хэ",
   CosimoDeMedici: "Козимо де Медичи",
   JamesWatt: "Джеймс Уатт",
   KarlMarx: "Карл Маркс",
   AdaLovelace: "Ада Лавлейс",
   NapoleonBonaparte: "Наполеон Бонапарт",
   CharlesDarwin: "Чарльз Дарвин",
   FlorenceNightingale: "Флоренс Найтингейл",
   FlorenceNightingaleDesc: "+%{value} Счастья",
   CharlesDarwinDesc: "+%{value} Науки от Занятых Рабочих",
   JuliusCaesar: "Юлий Цезарь",
   JuliusCaesarDesc: "+%{value} Науки от Незанятых Рабочих",
   Augustus: "Октавиан Август",
   CaiLun: "Цай Лунь",
   Cleopatra: "Клеопатра",
   SimaQian: "Сыма Цянь",
   Athens: "Афины",
   Rome: "Рим",
   DepositTileCountDesc: "%{count} плиток(и) %{deposit} может быть найдено в %{city}",
   Build: "Построить",
   ConstructionProgress: "Прогресс",
   ConstructionBuilderCapacity: "Мощность Строительства",
   ConstructionBuilderBaseCapacity: "Базовая Мощность",
   ConstructionBuilderMultiplier: "Множитель Мощности",
   ConstructionBuilderMultiplierFull: "Множитель Мощности Стоительства",
   EndConstruction: "Прекратить Стоительство",
   EndConstructionEnd: "Прекратить",
   EndConstructionDesc: "При прекращении строительства все уже использованные ресурсы не будут возвращены",
   Stonehenge: "Стоунхендж",
   StonehengeDesc:
      "Все здания, которые потребляют или производят камень, получают +1 к Множителю для Производства",
   LighthouseOfAlexandria: "Александрийский Маяк",
   LighthouseOfAlexandriaDesc: "Все примыкающие здания, получают +5 к Множителю для Хранилища",
   PyramidOfGiza: "Пирамида Гизы",
   PyramidOfGizaDesc: "Все здания, которые производят рабочих, получают +1 к Множителю для Производства",
   ColossusOfRhodes: "Колосс Родосский",
   ColossusOfRhodesDesc: "Все прилегающие здания, не производящие рабочих, получают +1 Счастье",
   MausoleumAtHalicarnassus: "Мавзолей в Галикарнасе",
   MausoleumAtHalicarnassusDesc: "Бесплатная транспортировка в пределах 2 плиток",
   HangingGarden: "Висячий сад",
   HangingGardenDesc:
      "+1 Множитель Производительности Строителей. Соседние акведуки получают +1 к производственному, складскому и рабочему Множителю для Производств",
   Colosseum: "Колизей",
   ColosseumDesc:
      "Мастерские Колесниц освобождаются от -1 Счастья. Поглощает 10 Колесниц и производит 10 Счастья",
   SummerPalace: "Летний дворец",
   SummerPalaceDesc:
      "Все прилегающие здания, потребляющие или производящие Порох, освобождаются от -1 Счастья. Все здания, потребляющие или производящие Порох, получают +1 Production, Storage and Worker Capacity",
   Rijksmuseum: "Рейксмузеум",
   RijksmuseumDesc:
      "+5 Счастья. Все здания, что производят или потребляют Культуру получают +1 Production, Storage and Worker Capacity",
   EiffelTower: "Эйфелева Башня",
   EiffelTowerDesc:
      "Все прилегающие Сталелитейные Заводы получают +N Production, Storage and Worker Multiplier. N = Число прилегащих Сталелитейных Заводов",
   CircusMaximus: "Цирк Максимус",
   CircusMaximusDesc:
      "+5 Счастья. Все Гильдии Актеров, Гильдии Писателей и Гильдии Художников получают +1 Production and Storage Multiplier",
   Statistics: "Здание Статистики",
   StatisticsDesc: "Предоставляет статистику потребления и производства",
   GrottaAzzurra: "Гротта Аццурра",
   GrottaAzzurraDesc: "При обнаружении все ваши здания 1-го Тира получают +5 Уровней",
   StatueOfZeus: "Статуя Зевса",
   StatueOfZeusDesc:
      "Производит на прилегающих плитках случайные месторождения, которые были обнаружены. Все прилегающие здания 1-го Тира получают +5 Production and Storage Multiplier",
   Poseidon: "Посейдон",
   PoseidonDesc: "Все прилегающие здания бесплатно улучшаются до 20 уровня",
   Aphrodite: "Афродита",
   AphroditeDesc: "+1 Builder Capacity Multiplier за каждый уровень при улучшении здания выше 20-го уровня",
   Alps: "Альпы",
   AlpsDesc:
      "Каждый 10-й уровень здания получает +1 Production Capacity (+1 Consumption Multiplier, +1 Production Multiplier)",
   GreatDagonPagoda: "Великая Пагода Дагона",
   GreatDagonPagodaDesc: "Great Dagon Pagoda",
   SagradaFamília: "Саграда Фамилия",
   SagradaFamíliaDesc: "Sagrada Família",
   Petra: "Петра",
   PetraDesc:
      "Создаёт временной разрыв, когда вы не в сети, который вы можете использовать для ускорения вашей империи",
   OxfordUniversity: "Оксфордский Университет",
   OxfordUniversityDesc: "Каждое улучшение зданий в вашей империи добавляет +1 Науку за цикл",
   StPetersBasilica: "Собор Святого Петра",
   StPetersBasilicaDesc:
      "Все здания, которые производят Веру, получают +1 Storage Multiplier. Производит Веру на основе Веры в хранилище",
   ForbiddenCity: "Запретный Город",
   ForbiddenCityDesc:
      "All Paper Makers, Writer's Guilds and Printing Houses get +1 Production Multiplier, Worker Capacity Multiplier and Storage Multiplier",
   Borobudur: "Боробудур",
   BorobudurDesc: "Borobudur",
   ItsukushimaShrine: "Храм Ицукусима",
   ItsukushimaShrineDesc: "Itsukushima Shrine",
   TempleOfArtemis: "Храм Артемиды",
   TempleOfArtemisDesc:
      "All Sword Forges and Armories get +5 Level when completed. All Sword Forges and Armories get +1 Production Multiplier, Worker Capacity Multiplier and Storage Multiplier",
   TempleOfHeaven: "Храм Неба",
   TempleOfHeavenDesc: "All buildings that are level 10 or higher get +1 Worker Capacity Multiplier",
   HimejiCastle: "Замок Химедзи",
   HimejiCastleDesc:
      "All Caravel Builders, Galleon Builders, and Frigate Builders get +1 Production Multiplier, Worker Capacity Multiplier and Storage Multiplier",
   TajMahal: "Тадж-Махал",
   TajMahalDesc:
      "Рождается новый великий человек Средневековья. +5 Builder Capacity Multiplier when upgrading buildings over Level 20",
   StatueOfLiberty: "Статуя Свободы",
   StatueOfLibertyDesc:
      "All adjacent buildings get +N Production, Storage and Worker Capacity Multiplier. N = Number of its adjacent buildings of the same type",
   Moai: "Моаи",
   MoaiDesc: "Moai",
   BranCastle: "Замок Бран",
   BranCastleDesc: "Bran Castle",
   Pantheon: "Пантеон",
   PantheonDesc: "Pantheon",
   SaintBasilsCathedral: "Храм Василия Блаженного",
   SaintBasilsCathedralDesc: "Saint Basil's Cathedral",
   ChichenItza: "Чичен-Ица",
   ChichenItzaDesc:
      "All adjacent buildings get +1 Production Multiplier, Storage and Worker Capacity Multiplier",
   Aqueduct: "Акведук",
   FishPond: "Рыбный Пруд",
   GarumMaker: "Производитель Гарума",
   Garum: "Гарум",
   Furniture: "Мебель",
   Opera: "Опера",
   Poem: "Поэма",
   Flour: "Мука",
   Fish: "Рыба",
   GreatMosqueOfSamarra: "Великая мечеть Самарры",
   GreatMosqueOfSamarraDesc: "Great Mosque Of Samarra",
   HagiaSophia: "Собор Святой Софии",
   HagiaSophiaDesc:
      "Здания с 0% Производственной Мощностью освобождаются от -1 Счастья. Потребляет 10 Веры И производит 10 Счастья",
   AngkorWat: "Ангкор-Ват",
   AngkorWatDesc: "Все примыкающие здания получают +1 Worker Capacity Multiplier. Производит 1000 Рабочих",
   TerracottaArmy: "Терракотовая Армия",
   TerracottaArmyDesc:
      "All Iron Mining Camps get +1 Production Multiplier, Worker Capacity Multiplier and Storage Multiplier. Iron Forges get +1 Production Multiplier for each adjacent Iron Mining Camp",
   Parthenon: "Парфенон",
   ParthenonDesc:
      "Новый Великий Человек Классического Века был рождён. Musician's Guilds and Painter's Guilds get +1 Worker Capacity Multiplier",
   Persepolis: "Персеполис",
   PersepolisDesc:
      "All Copper Mining Camps, Logging Camps and Stone Quarries get +1 Production Multiplier, Worker Capacity Multiplier and Storage Multiplier",
   ChoghaZanbil: "Чога Занбиль",
   ChoghaZanbilDesc: "Chogha Zanbil",
   HatshepsutTemple: "Храм Хатшепсут",
   HatshepsutTempleDesc:
      "Открывает все водные плиты на карте после завершения строительства. Пшеничные фермы получают +1 Множитель Производства за каждую прилегающую водную плиту",
   LuxorTemple: "Храм Луксор",
   LuxorTempleDesc: "+1 Наука от Занятых Рабочих",
   Neuschwanstein: "Нойшванштайн",
   NeuschwansteinDesc: "+10 Множитель Мощности Строительства при строительстве чудес",
   CristoRedentor: "Христос-Искупитель",
   CristoRedentorDesc: "Cristo Redentor",
   GoldenGateBridge: "Мост Золотые Ворота",
   GoldenGateBridgeDesc: "Golden Gate Bridge",
   BrandenburgGate: "Бранденбургские Ворота",
   BrandenburgGateDesc:
      "All coal mines and oil wells get +1 Production, Storage and Worker Capacity Multiplier. Oil refineries get +1 Production, Storage and Worker Capacity Multiplier for each adjacent oil tile",
   ArcDeTriomphe: "Триумфальная Арка",
   ArcDeTriompheDesc: "Arc de Triomphe",
   SydneyOperaHouse: "Sydney Opera House",
   SydneyOperaHouseDesc: "Сиднейский Оперный Театр",
   MogaoCaves: "Пещеры Могао",
   MogaoCavesDesc: "Mogao Caves",
   Wonder: "Чудо",
   PauseConstructionPause: "Пауза",
   ResumeConstructionResume: "Продолжить",
   Upgrade: "Улучшить",
   CancelUpgrade: "Отменить Улучшение",
   CancelUpgradeDesc: "Все ресурсы, которые уже были перемещены, останутся в хранилище",
   StockpileSettings: "Входная Мощность Хранилища",
   SellBuilding: "Продать Здание",
   StockpileDesc:
      "Это здание будет перевозить %{capacity}x входных ресурсов за производственный цикл до достижения максимального значения",
   StockpileMax: "Максимальный запас",
   StockpileMaxUnlimited: "Неограниченно",
   StockpileMaxDesc:
      "Это здание перестанет перевозить ресурс, как только его станет достаточно для %{cycle} производственных циклов",
   StockpileMaxUnlimitedDesc:
      "Здание никогда не прекратит транспортировку ресурсов, пока хранилище не будет полным",
   ProductionPriority: "Приоритет Производства",
   DefaultProductionPriority: "Приоритет Производства По Умолчанию",
   ConstructionPriority: "Приоритет Строительства",
   DefaultConstructionPriority: "Приоритет Строительства По Умолчанию",
   DefaultUpgradePriority: "Приоритет Улучшения По Умолчанию",
   ProductionPriorityDesc:
      "Высокий приоритет означает, что здание транспортирует ресурсы, раньше других зданий",
   TechProductionPriority:
      "Разблокирует приоритет зданий - позволяет установить приоритет производства для каждого здания",
   TechStockpileMode: "Разблокирует режим запасов - это позволит настраивать запасы для каждого здания",
   ConstructionCost: "Стоимость Строительства: %{cost}",
   RomeMapMenu: "Карта",
   PlayerMapMenu: "Торговля",
   Milk: "Молоко",
   Pizza: "Пицца",
   SiegeRam: "Осадный Таран",
   Caravel: "Каравелла",
   Galleon: "Галеон",
   Frigate: "Фрегат",
   CaravelBuilder: "Верфь Каравелл",
   GalleonBuilder: "Верфь Галеонов",
   FrigateBuilder: "Верфь Фрегатов",
   RifleFactory: "Фабрика Винтовок",
   Rifle: "Винтовка",
   GatlingGunFactory: "Фабрика Гатлингов",
   GatlingGun: "Гатлинг",
   IroncladBuilder: "Верфь Броненосцев",
   Ironclad: "Броненосец",
   Cotton: "Хлопок",
   CottonPlantation: "Хлопковая Плантация",
   CottonMill: "Хлопчатобумажный Комбинат",
   Armory: "Арсенал",
   SiegeWorkshop: "Осадная Мастерская",
   Castrum: "Каструм",
   Sword: "Меч",
   Armor: "Доспехи",
   Chariot: "Колесница",
   Province: "Провинция",
   Progress: "Прогресс",
   Apartment: "Апартаменты",
   Brewery: "Пивоварня",
   Warehouse: "Склад",
   WarehouseDesc: "Transport specific resources and provide extra storage",
   ProvinceHispania: "Hispania",
   ProvinceCantabri: "Cantabri",
   ProvinceGallia: "Gallia",
   ProvinceGalliaTransalpina: "Gallia Transalpina",
   ProvinceGalliaCisalpina: "Gallia Cisalpina",
   ProvinceItalia: "Italia",
   ProvinceIllyricum: "Illyricum",
   ProvinceMacedonia: "Macedonia",
   ProvinceSardiniaAndCorsica: "Sardinia And Corsica",
   ProvinceSicillia: "Sicillia",
   ProvinceBithynia: "Bithynia",
   ProvinceAsia: "Asia",
   ProvinceGalatia: "Galatia",
   ProvincePontus: "Pontus",
   ProvinceCappadocia: "Cappadocia",
   ProvinceCilicia: "Cilicia",
   ProvinceSophene: "Sophene",
   ProvinceLycia: "Lycia",
   ProvinceCommagene: "Commagene",
   ProvinceJudia: "Judia",
   ProvinceCreta: "Creta",
   ProvinceSyria: "Syria",
   ProvinceCyprus: "Cyprus",
   ProvinceAegyptus: "Aegyptus",
   ProvinceCyrene: "Cyrene",
   ProvinceAfrica: "Africa",
   ProvinceNumidia: "Numidia",
   ProvinceMauretania: "Mauretania",
   Chat: "Чат",
   ChatReconnect: "Отсоединено, подключение...",
   ChatNoMessage: "Нет Сообщений Чата",
   ChatSend: "Отправить",
   PlayerHandle: "Игровой Аккаунт",
   ChangePlayerHandle: "Изменить",
   ChangePlayerHandleCancel: "Отменить",
   ChangePlayerHandledDesc:
      "Ваше игровое имя может содержать только 5 ~ 16 букв и цифр и должно быть уникальным",
   AGreatPersonIsBorn: "Рождается великий человек",
   BuildingName: "Название",
   BuildingTier: "Тир",
   BuildingCost: "Стоимость",
   GreatPeople: "Великий Человек",
   ManageGreatPeople: "Управление Великими Людьми",
   ManageWonders: "Управление Чудесами",
   GreatPeopleEffect: "Эффект",
   GreatPeopleName: "Имя",
   GreatPeopleAmount: "Количество",
   GreatPeopleThisRun: "Великие Люди Из Этого Пробега",
   GreatPeopleThisRunShort: "Этот Пробег",
   GreatPeoplePermanentShort: "Постоянные",
   PermanentGreatPeople: "Постоянные Великие Люди",
   PermanentGreatPeopleShowInModal: "Показать во всплывающем окне",
   GreatPeopleWiki: "Википедия Великих Людей",
   GoBack: "Вернуться назад",
   CivIdle: "CivIdle",
   CivIdleInfo: "С гордостью представлено Fish Pond Studio",
   CivIdleBuild: "Версия: %{build}",
   GraphicsDriver: "Графический Драйвер: %{driver}",
   UserAgent: "Пользовательский Клиент: %{driver}",
   MarketYouPay: "You Pay",
   MarketYouGet: "Вы получите",
   MarketValueDesc: "%{value} compared to average price",
   MarketSell: "Продать",
   ResourceAmount: "Количество",
   RomanForum: "Римский Форум",
   Acropolis: "Акрополь",
   CensorsOffice: "Офис Цензора",
   StatisticsOffice: "Офис Статистики",
   NaturalWonderName: "Природное Чудо: %{name}",
   NaturalWonders: "Чудеса Природы",
   ShowTheoreticalValue: "Показать Теоритическое Значиение",
   StatisticsResources: "Ресурсы",
   StatisticsBuildings: "Здания",
   StatisticsTransportation: "Перевозка",
   StatisticsTransportationPercentage: "Процент Транспортных Рабочих",
   StatisticsTransportationBuilding: "Здание",
   StatisticsTransportationResource: "Ресурс",
   StatisticsTransportationAmount: "Кол.",
   PlayerTrade: "Торговля с Игроками",
   PlayerTradeIWant: "Я хочу",
   PlayerTradeSetMin: "Min",
   PlayerTradeSetMax: "Max",
   PlayerTradeSetHalf: "50%",
   PlayerTradeIOffer: "Я предлагаю",
   PlayerTradeResource: "Ресурс",
   PlayerTradeFillBy: "Продано",
   PlayerTradeAmount: "Количество",
   PlayerTradePlaceTrade: "Разместить заявку",
   PlayerTradeAddTradeCancel: "Отменить",
   PlayerTradeNewTrade: "Новая Заявка",
   PlayerTradeWant: "Хочу",
   PlayerTradeOffer: "Предлагаю",
   PlayerTradeFrom: "От",
   PlayerTradeFill: "Продать",
   PlayerTradeClaim: "Забрать",
   PlayerTradeFillAmount: "Количество к Продаже",
   PlayerTradeFillSuccess: "Заявка была успешно выполнена",
   PlayerTradeAddSuccess: "Заявка была успешно добавлена",
   PlayerTradeFillAmountMax: "Продать Всё Доступное",
   PlayerTradeFillTradeTitle: "Продать",
   PlayerTradeFillTradeButton: "Продать",
   PlayerTradeMaxTradeExceeded:
      "Вы превысили максимальное количество активных сделок для вашего уровня аккаунта",
   PlayerTradeCancelTrade: "Отменить Заявку",
   PlayerTradeCancelTradeNo: "Не Отменять",
   PlayerTradeCancelTradeNotEnoughStorage: "У вас недостаточно хранилища, чтобы отменить эту сделку",
   PlayerTradeCancelDesc:
      "При отмене этой сделки будет возвращен только %{percent} ресурсов. Вы уверены, что хотите отменить?",
   PlayerMapUnclaimedTile: "Незанятая Плитка",
   PlayerMapClaimThisTile: "Занять Эту Плитку",
   PlayerMapClaimTileCondition2: "Вы не были забанены античитом",
   PlayerMapClaimTileCondition3: "Вы разблокировали необходимую технологию: %{tech}",
   PlayerMapClaimTileCondition4: "Вы не заняли плитку или прошло время ожидания перемещения вашей плитки",
   PlayerMapClaimTileNoLongerReserved:
      "This tile is no longer reserved. You can evict <b>%{name}</b> and claim this tile for yourself",
   PlayerMapClaimTileCooldownLeft: "Время ожидания осталось: %{time}",
   PlayerMapYourTile: "Ваша Плитка",
   PlayerMapTariff: "Налог",
   PlayerMapSetYourTariff: "Установить Ваш Налог",
   PlayerMapTariffDesc:
      "Каждый торговец, проходящий через вашу плитку, будет платить вам налог. Здесь есть баланс: если вы увеличите тариф, то получите больше выгоды от каждой сделки, но меньше торговцев пройдет через вашу плитку.",
   PlayerMapTariffApply: "Применить Налоговую Ставку",
   PlayerMapTradesFrom: "Заявки от %{name}",
   PlayerMapEstablishedSince: "Основано в",
   PlayerMapLastSeenAt: "Last Seen",
   OfflineErrorMessage: "Вы сейчас оффлайн, для выполнения этой операции требуется подключение к Интернету",
   ResourceImport: "Транспортировка Ресурсов",
   ResourceImportResource: "Ресурсы",
   ResourceImportStorage: "Хранится",
   ResourceImportImportPerCycle: "Импорт",
   ResourceImportImportCap: "Предел",
   ResourceImportSettings: "Транспортировка Ресурса: %{res}",
   PlayerTradeYouPay: "Вы Платите: %{res}",
   PlayerTradeYouGetGross: "Вы Получаете (До Налога): %{res}",
   PlayerTradeYouGetNet: "Вы Получаете (После Налога): %{res}",
   PlayerTradeStorageRequired: "Хранилища Требуется",
   PlayerTradeClaimAvailable: "%{count} Заявок было выполнено и доступно для забора",
   PlayerTradeClaimTileFirst: "Займите свою плитку на торговой карте",
   PlayerTradeClaimTileFirstWarning:
      "Торговать с другими игроками можно только после того, как вы заняли свою плитку на торговой карте.",
   ApplyToAll: "Применить Для Всех",
   ApplyToAllBuilding: "Применить Для Всех %{building}",
   SetAsDefault: "Установить По Умолчанию",
   SetAsDefaultBuilding: "Установить по Умолчанию Для Всех %{building}",
   NothingHere: "Здесь ничего нет",
   ContentInDevelopment: "Контент В Разработке",
   ContentInDevelopmentDesc:
      "Этот игровой контент все еще находится в разработке и будет доступен в будущем обновлении игры, следите за новостями!",
   PatchNotes: "Примечания к Патчам",
   PlayerTradeNoValidRoute: "Невозможно найти подходящий торговый маршрут между вами и %{name}",
   BuildingColor: "Цвет Здания",
   ResourceColor: "Цвет Ресурса",
   NextMarketUpdateIn: "Следующее Обновление Рынка через",
   Theme: "Тема",
   Shortcut: "Горячие Клавиши",
   Gameplay: "Гемплей",
   Sound: "Звук",
   SoundEffect: "Звуковые Эффекты",
   ThemeColor: "Цвет Темы",
   ThemeColorReset: "Сбросить по Умолчанию",
   ThemeColorResetBuildingColors: "Reset Building Colors",
   ThemeColorResetResourceColors: "Reset Resource Colors",
   ThemeColorWorldBackground: "Фон Мира",
   ThemeColorResearchBackground: "Фон Исследования",
   ThemeColorGridColor: "Цвет Плитки",
   ThemeColorGridAlpha: "Яркость Граней",
   ThemeSelectedGridColor: "Цвет Выбранной Плитки",
   ThemeInactiveBuildingAlpha: "Яркость Неработающих Зданий",
   ThemeTransportIndicatorAlpha: "Яркость Значков Транспорта",
   ThemeResearchLockedColor: "Цвет ",
   ThemeResearchUnlockedColor: "Цвет Закрытого Исследования",
   ThemeResearchHighlightColor: "Цвет Выбранного Исследования",
   Happiness: "Счастье",
   HappinessFromUnlockedTech: "От Открытых Исследований",
   HappinessFromUnlockedAge: "От Открытых Эпох",
   HappinessFromBuildingTypes: "От Зданий С Полным Хранилищем",
   HappinessFromHighestTierBuilding: "От Рабочего Здания Высшего Тира",
   HappinessFromBuilding: "От Зданий (Исключая Чудеса)",
   HappinessFromWonders: "От Чудес (Включая Природные)",
   ShortcutScopeBuildingPage: "Страница Зданий",
   ShortcutScopeTechPage: "Страница Технологий",
   ShortcutScopeEmptyTilePage: "Страница Пустой Плитки",
   ShortcutScopeUpgradePage: "Upgrade Page",
   ShortcutScopePlayerMapPage: "Trade Map Page",
   ShortcutBuildingPageSellBuilding: "Продать Здание",
   ShortcutBuildingPageUpgradeX1: "Улучшить x1",
   ShortcutBuildingPageUpgradeX5: "Улучшить x5",
   ShortcutBuildingPageUpgradeToNext10: "Улучшить до кратного 10",
   ShortcutUpgradePageIncreaseLevel: "Increase Upgrade Level",
   ShortcutUpgradePageDecreaseLevel: "Decrease Upgrade Level",
   ShortcutNone: "Нет",
   ShortcutClear: "Очистить",
   ShortcutSave: "Сохранить",
   ShortcutPressShortcut: "Нажмите на клавишу...",
   ShortcutConflict: "Ваша Горячая Клавиша пересекается с %{name}",
   ShortcutTechPageGoBackToCity: "Вернуться В Город",
   ShortcutTechPageUnlockTech: "Unlock Selected Tech",
   EmptyTilePageBuildLastBuilding: "Построить Последнее Здание",
   PlayerMapPageGoBackToCity: "Go Back To City",
   UnclaimedGreatPerson: "У вас есть незабранные Великие Люди, Нажмите здесь, чтобы забрать",
   OfflineProduction: "Оффлайн Продукция",
   OfflineTime: "Время Оффлайн",
   TransportCapacity: "Мощность Транспорта",
   TransportCapacityMultiplier: "Множитель Мощности Транспорта",
   WonderBuilderCapacityDesc:
      "Мощность Строительства при постройке Чудес зависит от эпохи и технологий, которые открыли Чудеса",
   MarketSettings: "Настройки Рынка",
   ClearAfterUpdate: "Очищать Всю Торговлю После Обновления Рынка",
   WarehouseSettings: "Настройки Склада",
   WarehouseSettingsAutopilot: "Режим Автопилота",
   WarehouseSettingsAutopilotDesc:
      "Этот склад будет использовать свои незадействованные мощности для транспортировки ресурсов из зданий, которые имеют полное хранилище. Текущая незанятая емкость: %{capacity}",
   WarehouseUpgrade:
      "Разблокирует режим автопилота для склада. Бесплатная транспортировка между Складом и Примыкающими плитками",
   WarehouseUpgradeDesc: "Бесплатная транспортировка между Складом и Примыкающими плитками",
   Warp: "Временной Разрыв",
   MaxOfflineProductionTime: "Максимальное Время Оффлайна",
   WarpConversionDesc:
      "Время Оффлайна, превышающее максимальное время Оффлайна, будет преобразовано во Временной Разрыв (1 Секунда = 1 Временной Разрыв)",
   TimeWarp: "Ускорение Времени",
   TurnOnTimeWarpDesc:
      "Стоит %{speed} Временного Разрыва за каждую секунду и ускоряет вашу империю в %{speed}x раза.",
   TimeWarpWarning:
      "Ускорение на более высокой скорости, чем может выдержать ваш компьютер, может привести к потере данных.: ИСПОЛЬЗУЙТЕ НА СВОЙ СТРАХ И РИСК",
   PetraNoMultiplier:
      "На Петру не действуют никакие множители (продукции, мощности рабочих, хранилища, и т.д.)",
   LimitOfflineProduction: "Ограничение Оффлайн Продукции",
   Reborn: "Переродиться",
   RebornCity: "Выбрать Город Для Перерождения",
   RebornTradeWarning:
      "You have trades that are active or can be claimed. <b>Rebirth will erase them</b> - you should consider cancelling or claiming first",
   RebornOfflineWarning:
      "You are currently offline. You can only rebirth when you are connected to the server",
   TotalEmpireValue: "Общая Стоимость Империи",
   ProgressTowardsNextGreatPerson: "Progress Towards Next Great Person at Reborn",
   NotProducingBuildings: "Здания Которые Не Производят",
   ExtraGreatPeopleAtReborn: "Дополнительные Великие Люди После Перерождения",
   Ok: "ОК",
   Cancel: "Отменить",
   ChooseGreatPersonFromLastReborn: "Выберите Великого Человека Из Последнего Перерождения",
   BankingAdditionalUpgrade: "Все здания 10 уровня или выше получают +1 к Множителю Хранилища",
   EstimatedTimeLeft: "Осталось примерно",
   Electrification: "Электрификация",
   ElectrificationUpgrade:
      "Разблокирует электрификацию. Позволяет зданиям потреблять энергию для увеличения производства",
   ElectrificationDesc:
      "Использует %{power}W Энергии и Даёт +%{level} Множитель Потребления и Множитель Производства",
   PowerAvailable: "Энергии Всего",
   PowerUsed: "Энергии Использовано",
   Power: "Энергия",
   ElectrificationStatus: "Текущий Статус",
   ElectrificationStatusNotActive: "Не Активно",
   ElectrificationStatusNoPower: "Нет Энергии",
   ElectrificationStatusActive: "Активно",
   ConfirmYes: "Да",
   ConfirmNo: "Нет",
   ConfirmDestroyResourceContent: "Вы собираетесь уничтожить %{amount} %{resource}. Это нельзя отменить",
   BuildingPriority: "Приоритет Зданий",
   ChatChannel: "Канал Чата",
   ChatChannelLanguage: "Язык",
   ChatChannelSend: "Отравить",
   ChatChannelReceive: "Получить",
   FirstTimeGuideNext: "Дальше",
   FirstTimeGuideFinish: "Завершить",
   AccountTypeShowDetails: "Показать Детали Аккаунта",
   PlayerHandleOffline: "Вы сейчас не в сети",
   AccountLevel: "Уровень Аккаунта",
   TrialRunDesc:
      "Квестор или выше может начать пробный запуск, который подчиняется ограничениям Трибуна и не может заработать постоянных великих людей.",
   Tutorial1:
      "Добро Пожаловать в CivIdle.<br><br>В этой игре вы будете управлять своей собственной империей: управлять производством, открывать технологии, торговать ресурсами с другими игроками, создавать великих людей и строить чудеса.<br><br>Перетаскивайте мышь для перемещения по игре. Используйте колесо прокрутки для увеличения или уменьшения масштаба. Щелкните по пустой клетке, чтобы построить новые здания, или щелкните по зданию для просмотра информации о нем.<br><br>",
   Tutorial2: "Я не играл в подобные игры и хотел бы пройти краткое руководство",
   Tutorial3:
      "Я играл в Industry Idle и знаю, как это делается, поэтому, пожалуйста, хочу пропустить базовое руководство.",
   Tutorial4:
      "Здания - это основной строительный блок вашей империи - поместите их на исследованные плитки карты. Некоторые здания, такие как Каменные Карьеры и Акведуки, которые добывают ресурсы, должны быть размещены на плитке с месторождением. Здание убирает туман с окружающих его плиток.<br><br>Здания потребляют ресурсы и производят новые. Они будут <b>автоматически</b> транспортировать нужные ресурсы. Произведенные ресурсы будут храниться в зданиях, которые могут быть транспортированы в другие здания, которым нужны эти ресурсы. Однако хранилище в зданиях ограничено.<br><br>Зданиям нужны <b>рабочие</b> для транспортировки и производства - вам нужно построить жилые дома, такие как <b>хижины</b>, чтобы увеличить количество рабочих. Вы можете улучшить здание, чтобы увеличить его производство и хранение.",
   Tutorial5:
      "<b>Наука</b> используется для разблокировки технологий в дереве технологий. Технологии открывают новые здания, чудеса и усиления.<br><br><b>Занятые Рабочие</b> создают небольшое количество науки. Позже вы откроете такие здания, как библиотеки и школы, которые производят науку.<br><br>На количество доступных работников влияет <b>счастье</b>. Каждое здание будет стоить 1 счастье. И вы получаете счастье от технологий, чудес и великих людей. Следите за тем, чтобы ваши работники были довольны, иначе производство в вашей империи остановится.<br><br><b>Чудеса</b> это единственные в своем роде здания - вы можете построить только одно, и их нельзя удалить после постройки. Чудеса могут дать значительное усиление или открыть новые механизмы.",
   Tutorial6:
      "Когда вы вступаете в новую эпоху, рождается новый <b>великий человек</b>. Вы можете выбрать одного из трех кандидатов.<br><br>Великие люди дают бонусы и добавляются в вашу постоянную коллекцию. Когда вы перерождаетесь, вы собираете всех великих людей, которых вы выбрали в этом раунде, плюс некоторых ушедших, эти постоянные великие люди останутся с вами.<br><br><b>Постоянные</b> Великие люди могут быть улучшены, если собирать великих людей из каждом пробеге. Они помогут вашей империи достичь новых высот. Вот и все основы, которые вам нужно знать.<br><br>Если у вас есть вопросы, лучше всего обратиться за помощью к <b>чату</b> - у нас дружное сообщество, которое всегда готово помочь.",
   TutorialPlayerHandle: "Выберите ваше игровое имя",
   TutorialPlayerFlag: "Выберите ваш игровой флаг",
   JoinDiscord: "Присоединяйтесь К Дискорду",
   AccountLevelTribune: "Трибун",
   AccountLevelQuaestor: "Квестор",
   AccountLevelAedile: "Эдил",
   AccountLevelPraetor: "Претор",
   AccountLevelConsul: "Консул",
   AccountLevelCensor: "Цензор",
   AccountChatBadge: "Значок Чата",
   AccountLeaderboard: "Таблица Лидеров",
   AccountActiveTrade: "Активных Заявок",
   AccountTradeValuePerMinute: "Торговой Стоимости В Минуту",
   AccountTradePriceRange: "Диапазон Цен Торговли",
   AccountTradeTileReservationTime: "Резервирование Торговой Плитки",
   AccountTradeTileReservationTimeDesc:
      "Это время, в течение которого ваша торговая плитка будет зарезервирована за вами, с тех пор как вы последний раз были онлайн. После окончания периода резервирования, ваша плитка станет доступной для других игроков",
   AccountUpgradeCarryOver: "Уровень Переноса",
   AccountUpgradeCarryOverDesc:
      "При переходе на следующий уровень аккаунта это максимальный уровень постоянных великих людей, который вы можете перенести",
   TranslationPercentage: "%{language} на %{percentage} переведён. Помогите улучшить этот перевод на GitHub",
   CanEarnGreatPeopleFromRebornYes: "Этот пробег может зарабатывать Великих Людей от перерождения",
   CanEarnGreatPeopleFromRebornNo: "Этот пробег не может зарабатывать Великих Людей от перерождения",
   TribuneUpgradeDesc:
      "Переход с Трибуна на Квестора позволит вашей учетной записи использовать все онлайн функции. Однако, <b>Вы можете перенести Постоянных Великих Людей только до уровня %{level}</b>",
   TribuneUpgradeDescGreatPeopleWarning:
      "Your current run has great people. You should <b>reborn first</b>. Upgrading to Quaestor rank will reset your current run",
   TribuneGreatPeopleLevelWarning:
      "Уровень вашей учетной записи Трибун - при переходе на Квестор, Вы можете перенести Постоянных Великих Людей до уровня %{level}",
   RebornModalDesc:
      "Вы начнете новую империю, но сможете забрать всех Великих Людей <b>из этого пробега</b>, плюс дополнительных великих людей на основе вашей <b>общей стоимости империи</b>",
   OperationNotAllowedError: "Эта операция не разрешена",
   AccountLevelUpgradeConditionAny:
      "Чтобы повысить уровень аккаунта, вам достаточно соответствовать одному из следующих критериев:",
   AccountLevelPlayTime:
      "Активное время в режиме онлайн > %{requiredTime} (Ваше игровое время: %{actualTime})",
   AccountUpgradeButton: "Улучшить аккаунт до Квестор",
   AccountUpgradeConfirm: "Улучшить Аккаунт",
   AccountUpgradeConfirmDesc:
      "Улучшение вашего аккаунта приведет к <b>сбросу текущего пробега</b> и переносу постоянных великих людей <b>до уровня %{level}</b>. Это <b>не может</b> быть отменено, Вы уверены, что хотите продолжить?",
   CannotEarnPermanentGreatPeopleDesc: "Because this is a trial run, permanent great people cannot be earned",
   BackToCity: "Back To City",
   UpgradeBuildingNotProducingDesc:
      "This building is being upgraded - production will halt until upgrade is complete",
   Tutorial: "Tutorial",
   SelectedCount: "%{count} Selected",
   SelectedAll: "Select All",
   InverseSelection: "Inverse",
   ClearSelection: "Clear",
   RedistributeAmongSelected: "Redistribute Among Selected",
   RedistributeAmongSelectedImport: "Import",
   RedistributeAmongSelectedCap: "Cap",
   PlayerTradeTariffTooltip: "Collected From a Trade Tariff",
   PlayerTradeClaimAll: "Claim All",
   PlayerTradeClaimAllMessage: "You have claimed %{count} trades",
   SteamAchievement: "Steam Achievement",
   SteamAchievementDetails: "View Steam Achievement",
};
