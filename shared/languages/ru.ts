export const RU = {
   ...
   EuphratesRiverDesc:
      "Каждые 10% занятых рабочих, занятых производством (не транспортировкой), дают +1 Множитель Производства всем зданиям, которые не производят рабочих (максимум = количество разблокированных эпох / 2). Когда рядом с ним строятся Висячие Сады, то Висячие Сады получают +1 эффект за каждую эпоху после их разблокировки. При обнаружении порождает воду на всех соседних плитках, на которых нет месторождений.",
   ...
   MountFujiDesc: "Когда Петра построена рядом с ним, Петра получает +8ч Хранилищу Временного Разрыва. Когда игра запущена, генерирует 30 Временного Разрыва каждую минуту в Петре (не ускоряется самой Петрой, не генерируется, когда игра офлайн)",
