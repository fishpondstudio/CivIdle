import { BuildingSpecial, type Building } from "../definitions/BuildingDefinitions";
import type { City } from "../definitions/CityDefinitions";
import { IsDeposit, NoPrice, type Deposit, type Material } from "../definitions/MaterialDefinitions";
...
export function calculateTierAndPrice(log?: (val: string) => void) {
               ...
               if (!Config.MaterialTier[res] || targetTier < Config.MaterialTier[res]!) {
                  ...
                  forEach(resourceTierDependency, (k, v) => {
                     if (v === res) {
                        ...
                        log?.(
                           `Resource Tier of ${k} is decided by ${res}, but its tier has changed from ${oldTier ?? "?"} to ${targetTier}`,
                        );
                     }
                  });
                  forEach(buildingTierDependency, (k, v) => {
                     if (v === res) {
                        ...
                        log?.(
                           `Building Tier of ${k} is decided by ${res}, but its tier has changed from ${oldTier ?? "?"} to ${targetTier}`,
                        );
                     }
                  });
               }
   ...
   Config.MaterialPrice.Koti = 10000000;
      ...
      .forEach((r) => {
         resourcePrice.push(
            `${r.padEnd(20)}${numberToRoman(Config.MaterialTier[r]!)!.padEnd(10)}${String(
               ...
               10,
            )}${String(resourcesUsedByWonder.get(r) ?? "0*").padEnd(10)}${resourcesUsedByBuildings.get(r) ?? "0*"}`,
         );
      });
...
export function getBuildingUnlockTechSlow(building: Building): Tech | null {
   let key: Tech;
   for (key in Config.Tech) {
      if (Config.Tech[key].unlockBuilding?.includes(building)) {
         return key;
...
export function getResourceUnlockTechs(res: Material): Tech[] {
...
export function getOrderedTechThatProduce(res: Material): Tech[] {
